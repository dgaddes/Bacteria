...
def degenerate(primer):
    ...
    for i in range(0, len(primer)):
        for j in range(0, len(forward)):
            ...
            if primer[i] == 'N':
                forward = N_convert(forward, primer, i)
            elif primer[i] == 'Y':
                forward = Y_convert(forward, primer, i)
            else:
                pass
    return forward

def degenerate2(primer):
    forward = (str(primer),)
    for i in range(0, len(primer)):
        for j in range(0, len(forward)):
            primer = MutableSeq(forward[j])
            if (primer[i] == 'A') or (primer[i] == 'C') or (primer[i] == 'G') or (primer[i] == 'T'):
                ...
                forward = convert(forward, primer, i, primer[i])
    return forward

def convert(forward, primer, i, letter):
    R = ['A', 'G', 'R']
    M = ['A', 'C', 'M']
    S = ['C', 'G', 'S']
    B = ['C', 'G', 'T', 'B']
...
def Y_convert(forward, primer, i):
    primer[i] = "C"
    forward = forward + (str(primer),)
    primer[i] = "T"
    forward = forward + (str(primer),)
...
def get_amplicon(core_data, rev_tup, fwd_tup):
    add_on_data = []
    all_sequnces = []
...
def PCR_amplify(core_data, rev_tup, fwd_tup):
    add_on_data = []
    all_sequnces = []
    for item in core_data['Species']:
        #a = core_data.loc[core_data['Species'] == item]
